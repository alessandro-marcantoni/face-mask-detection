...
class FaceMaskDetector:
    ...
    def __init__(self):
        ...
        self.face_target_shape = (640, 640)
        self.face_detector = load_model("models/detector")
        #self.face_summary = self.face_detector.count_params()

        # Definition of the Mask Detection Interpreter.
        self.mask_target_shape = (224, 224)
        self.mask_detector = load_model("models/mask-detector-model.model")
    
    def summaries(self):
        print("[ NUMERO DI PARAMETRI FACE DETECTOR:" + str(self.face_detector.count_params()) + "]")
        self.face_detector.summary()
        print("[ NUMERO DI PARAMETRI MASK DETECTOR:" + str(self.mask_detector.count_params()) + "]")
        self.mask_detector.summary()
    ...
    def detect_faces(self, image):
        ...
        image_for_net = np.expand_dims(image_for_net, 0)

        with tf.device("/gpu:0"):
            bboxes = self.face_detector.inference(image_for_net)
    ...
    def face_mask_inference(self, image):
        ...
        with tf.device("/gpu:0"):
            probabilities = self.mask_detector.predict(image_tensor)
